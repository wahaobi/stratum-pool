...
function DaemonInterface(daemons, logger){
    ...
    function performHTTPSRequest(jsonData, callback) 
    {
        var url = 'https://pool.biblepay.org/Action.aspx?' + jsonData;
        var data = '';
        ...
        var req = https.get(url, function(resp) 
        {
            ...
            {
                  var iEnd = data.indexOf('}') + 1;
                  var myData = data.substring(0, iEnd);
                  parseJson(resp, myData);
            });
    ...
    this.performHTTPSRequest = performHTTPSRequest;
}
